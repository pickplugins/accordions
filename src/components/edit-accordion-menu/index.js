...
function Html(props) {
	...
	const [setnavMenus, setsetnavMenus] = useState([]);
	...
	useEffect(() => {
		...
		apiFetch({
			path: "/accordions/v2/get_nav_menus",
			method: "POST",
			data: {
			...
			var navMenusX = [];
			res.map((item) => {
				navMenusX.push({ label: item.label, value: item.id });
			});
			setnavMenus(navMenusX);
					...
					<PanelBody
						...
						<div className="my-4 flex items-center justify-between ">
							...
							<div className="flex items-center  gap-2">
								...
								{globalOptions?.itemSource == "navMenus" && (
									<>
										<span
											className="cursor-pointer"
											title="Click to know more"
...
class AccordionMenuEdit extends Component {
	constructor(props) {
		super(props);
...
export default AccordionMenuEdit;
