...
const PgCSSLeft = ({ val, onChange, breakPoint, sudoSrc }) => {
	var args = {
					...
					{unit != "auto" && (
						<InputControl
							...
							onChange={(newVal) => {
								...
								if (unit == "auto") {
									if (isImportant) {
										onChange(unit + " !important", "left");
									} else {
										onChange(unit, "left");
									}
								} else {
									if (isImportant) {
										onChange(v + unit + " !important", "left");
									} else {
										onChange(v + unit, "left");
									}
								}
							}}
						/>
						...
						<select
							...
							onChange={(ev) => {
								...
								if (selectedValue == "auto") {
									if (isImportant) {
										onChange(selectedValue + " !important", "left");
									} else {
										onChange(selectedValue, "left");
									}
								} else {
									if (isImportant) {
										onChange(valueX + selectedValue + " !important", "left");
									} else {
										onChange(valueX + selectedValue, "left");
									}
								}
							}}>
					...
					onChange={(arg) => {
						...
						if (isImportant) {
							if (unit == "auto") {
								...
								onChange(valueX + unit, "left");
							}
						} else {
							if (unit == "auto") {
								onChange(unit + " !important", "left");
							} else {
								onChange(valueX + unit + " !important", "left");
							}
						}
					}}
				/>
