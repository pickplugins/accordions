...
const PgCSSBottom = ({ val, onChange, breakPoint, sudoSrc }) => {
	var args = {
					...
					{unit != "auto" && (
						<InputControl
							...
							onChange={(newVal) => {
								...
								if (unit == "auto") {
									if (isImportant) {
										onChange(unit + " !important", "bottom");
									} else {
										onChange(unit, "bottom");
									}
								} else {
									if (isImportant) {
										onChange(v + unit + " !important", "bottom");
									} else {
										onChange(v + unit, "bottom");
									}
								}
							}}
						/>
						...
						<select
							...
							onChange={(ev) => {
								...
								if (selectedValue == "auto") {
									if (isImportant) {
										onChange(selectedValue + " !important", "bottom");
									} else {
										onChange(selectedValue, "bottom");
									}
								} else {
									if (isImportant) {
										onChange(valueX + selectedValue + " !important", "bottom");
									} else {
										onChange(valueX + selectedValue, "bottom");
									}
								}
							}}>
					...
					onChange={(arg) => {
						...
						if (isImportant) {
							if (unit == "auto") {
								...
								onChange(valueX + unit, "bottom");
							}
						} else {
							if (unit == "auto") {
								onChange(unit + " !important", "bottom");
							} else {
								onChange(valueX + unit + " !important", "bottom");
							}
						}
					}}
				/>
